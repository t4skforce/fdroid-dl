...
class Index(collections.MutableMapping):
    ...
    def findAppIds(self,key):
        if key is None: raise KeyError("key must not be empty")
        retVal = set()
    ...
    def run_custom_func(self,method,value):
        if method is None or value is None: return None
        try:
    ...
    def xmlattr(self,node,xml_node,key=None,xpath=None):
        if xpath is None and key is None: return None
        if xpath is None and not key is None: xpath = key
        value = self.xmlattrval(xml_node,xpath)
        if not value is None:
            if not key is None: node[key]=value
            else: node=value
    ...
    def xmlattrval(self,xml_node,xpath):
        if not xml_node is None:
            p = re.compile('^.*?\[@([^\]]+)\]$')
            m = p.match(xpath)
            if not m is None: attr_name = m.group(1)
            else: attr_name = xpath
    ...
    def convert(self,root):
        ...
        for xmlapp in applications:
            ...
            if not cat is None and len(cat) > 0: appVal['categories']=cat.split(',')
            afaet = self.xmltextval(xmlapp,'antiFeatures')
            if not afaet is None and len(afaet) > 0: appVal['antiFeatures']=afaet.split(',')
            self.xmltext(appVal,xmlapp,'suggestedVersionName','./marketversion')
            self.xmltext(appVal,xmlapp,'suggestedVersionCode','./marketvercode')
            ...
            self.xmltext(appVal,xmlapp,'added','./added:int(mktime(strptime(value, "%Y-%m-%d")))')
            self.xmltext(appVal,xmlapp,'icon','./icon')
            self.xmltext(appVal,xmlapp,'packageName','./id')
            self.xmltext(appVal,xmlapp,'lastUpdated','./lastupdated:int(mktime(strptime(value, "%Y-%m-%d")))')
            ...
            if not appId is None:
                ...
                for xmlpkg in packages:
                    pkgVal={}
                    self.xmltext(pkgVal,xmlpkg,'added','./added:int(mktime(strptime(value, "%Y-%m-%d")))')
                    ...
                    self.xmltext(pkgVal,xmlpkg,'targetSdkVersion','./targetSdkVersion')
                    pkgVal['packageName']=appId
                    self.xmltext(pkgVal,xmlpkg,'sig','./sig')
                    self.xmltext(pkgVal,xmlpkg,'versionName','./version')
                    self.xmltext(pkgVal,xmlpkg,'versionCode','./versioncode')
                    ...
                    if len(pkgVal) > 0: pkg.append(pkgVal)
            if len(appVal) > 1: apps.append(appVal)

    def monkeypatch(self):
        ''' fixup metadata paths -> url '''
        if not '_monkeypatched' in self.__store:
            if self.__key is None: raise AttributeError('key not defined')
                ...
                for key in self.__store['packages']:
                    for pkg in self.__store['packages'][key]:
                        ...
                        if not pkg.get('srcname') is None:
                            pkg['srcname'] = urljoin(self.__key,pkg['srcname'])

            if 'apps' in self.__store:
                for app in self.__store['apps']:
                    if 'icon' in app: app['icon']=urljoin(self.__key,"icons/"+app['icon'])
                    locs = app.get('localized',{})
                    for k in locs.keys():
                        loc = locs[k]
                        if 'icon' in loc:
                            loc['icon']=urljoin(self.__key,app['packageName']+'/'+k+'/'+loc['icon'])
                        if 'featureGraphic' in loc:
                            loc['featureGraphic']=urljoin(self.__key,app['packageName']+'/'+k+'/'+loc['featureGraphic'])
                        if 'promoGraphic' in loc:
                            loc['promoGraphic']=urljoin(self.__key,app['packageName']+'/'+k+'/'+loc['promoGraphic'])
                        if 'tvBanner' in loc:
                            loc['tvBanner']=urljoin(self.__key,app['packageName']+'/'+k+'/'+loc['tvBanner'])
                        if 'phoneScreenshots' in loc:
                            loc['phoneScreenshots']=[urljoin(self.__key,app['packageName']+'/'+k+'/phoneScreenshots/'+value) for value in loc['phoneScreenshots']]
                        if 'sevenInchScreenshots' in loc:
                            loc['sevenInchScreenshots']=[urljoin(self.__key,app['packageName']+'/'+k+'/sevenInchScreenshots/'+value) for value in loc['sevenInchScreenshots']]
                        if 'tenInchScreenshots' in loc:
                            loc['tenInchScreenshots']=[urljoin(self.__key,app['packageName']+'/'+k+'/tenInchScreenshots/'+value) for value in loc['tenInchScreenshots']]
                        if 'tvScreenshots' in loc:
                            loc['tvScreenshots']=[urljoin(self.__key,app['packageName']+'/'+k+'/tvScreenshots/'+value) for value in loc['tvScreenshots']]
                        if 'wearScreenshots' in loc:
                            loc['wearScreenshots']=[urljoin(self.__key,app['packageName']+'/'+k+'/wearScreenshots/'+value) for value in loc['wearScreenshots']]
        return self
    ...
    def save(self,filename=None):
        if not filename is None: self.__filename=filename
